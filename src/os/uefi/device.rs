...
fn get_protocol<P: Protocol>(handle: Handle) -> uefi::Result<&'static mut P> {
    let bs = uefi_services::system_table().boot_services();
    unsafe {
        use uefi::table::boot::{OpenProtocolAttributes, OpenProtocolParams};
        let image = crate::os::uefi::image_handle();

        let protocol = bs.open_protocol::<P>(
            OpenProtocolParams {
                handle,
                agent: image,
                controller: None,
...
fn esp_live_image(esp_handle: Handle, esp_device_path: &DevicePath) -> Option<Vec<u8>> {
    ...
    let mut live_image = match root.open(filename) {
        ...
        Err(err) => {
            log::warn!(
                "Failed to open {}\\redstone-live.iso: {:?}",
                ...
                DevicePathType::Media => match DevicePathMediaType::try_from(node.SubType) {
                    Ok(sub_type) => match sub_type {
                        DevicePathMediaType::Harddrive
                            if node_data.len() == core::mem::size_of::<DevicePathHarddrive>() =>
                        {
                            let harddrive = unsafe {
                                core::ptr::read(node_data.as_ptr() as *const DevicePathHarddrive)
                            };
                            ...
                            match harddrive.signature_type {
                                1 => {
                                    let id = unsafe {
                                        core::ptr::read(
                                            harddrive.partition_signature.as_ptr() as *const u32
                                        )
                                    };
                                ...
                                2 => {
                                    let guid = unsafe {
                                        core::ptr::read(
                                            harddrive.partition_signature.as_ptr() as *const Guid
                                        )
                                    };
                                    write!(
                                        s,
